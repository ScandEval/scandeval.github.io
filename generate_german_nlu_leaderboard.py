"""Python script to generate the German NLU leaderboard."""
...
BENCHMARK_ENTRY = """  <tr>
   ...
   <td class="de ner">{de_ner}</td> <!-- GermEval -->
   <td class="de sent">{de_sent}</td> <!-- SB10k -->
   <td class="de la">{de_la}</td> <!-- ScaLA-de -->
   <td class="de qa">{de_qa}</td> <!-- GermanQuAD -->
  </tr>"""
...
def main() -> None:
    ...
    benchmark_path = Path("german-nlu-benchmark.md")
    ...
    for model_id, model_dict in model_scores.items():
        values = dict(
            ...
            de_ner=model_dict.get("de ner", ""),
            de_sent=model_dict.get("de sent", ""),
            de_la=model_dict.get("de la", ""),
            de_qa=model_dict.get("de qa", ""),
        )
    ...
    # Log status
    logging.info(
        f"Generated German NLU benchmark with results from {len(model_scores):,} "

"""Python script to generate the Dutch NLU leaderboard."""
...
<table id="dutch-nlu-benchmark" class="sortable fixed centered small-font">
 <thead>
  <tr>
   ...
   <th><span data-toggle="tooltip" data-placement="bottom" data-container="body" title="Dutch sentiment classification - Matthews correlation coefficient / Macro-average F1-score">DutchSocial</span></th>
   ...
   <th><span data-toggle="tooltip" data-placement="bottom" data-container="body" title="Dutch question answering - Exact match / F1-score">???</span></th>
  </tr>
 </thead>
 <tbody>"""
...
BENCHMARK_ENTRY = """  <tr>
   ...
   <td class="nl ner">{nl_ner}</td> <!-- CoNLL-NL -->
   <td class="nl sent">{nl_sent}</td> <!-- DutchSocial -->
   <td class="nl la">{nl_la}</td> <!-- ScaLA-nl -->
   <td class="nl qa">{nl_qa}</td> <!-- ??? -->
  </tr>"""
...
def main() -> None:
    ...
    benchmark_path = Path("dutch-nlu-benchmark.md")
    ...
    for model_id, model_dict in model_scores.items():
        values = dict(
            ...
            nl_ner=model_dict.get("nl ner", ""),
            nl_sent=model_dict.get("nl sent", ""),
            nl_la=model_dict.get("nl la", ""),
            nl_qa=model_dict.get("nl qa", ""),
        )
    ...
    # Log status
    logging.info(
        f"Generated Dutch NLU benchmark with results from {len(model_scores):,} "
